...
from adapted_genetic_optimizer import optimizer
...
# RLV values
Rn     = 0.05          # nose radius (m)
Mo     = 5         # Mass at orbit (kg)
Mp     = 0          #  Mass of propellant (kg)
m = Mo + Mp            # total mass

# aerodynamic constant
S      = 0.01         # frontal area, m^2
...
# initial conditions:
h0, v0, gamma0, lamda0, phi0, psi0 = 120000,7200,0,0,0,0
...
# plot results
def plot(times, Y, a_values, dp_values, q_values):
    ...
    plt3 = plt.plot(q_values,cols[0], label="Heat Flux")
    ...
    plt4 = plt.plot(a_values, cols[0], label="Angle of attack (in deg)")
    ...
    plt5 = plt.plot(dp_values, cols[0], label="Dynamics pressure")
    plt.xlabel("Dynamics pressure (kPa)")
    plt.ylabel("Altitude")
    plt.legend()
    plt.grid()
    plt.show()
...
if __name__ == "__main__":
    ...
    a_values = []  # angle of attack
    dp_values = []  # Qp_kpa
    q_values = []   # q


    # integration time step 
    endtime = 800; t_step = 1;
    ...
    for t_start, t_end in zip(times, times[1:]):
        ...
        alpha_new, L_new, D_new, Qp_kpa_new, q_new = opt.get_values(Y, control, orb_data, tem_data, aero_data)

        # pack args list
        args = [alpha_new, L_new, D_new]
        control.append(args)
        a_values.append(alpha_new)
        dp_values.append(Qp_kpa_new)
    ...
    # Amendment
    a_values.append(0.0)
    dp_values.append(0.0)
    q_values.append(0.0)


    # plot the results
    plot(times, Y, a_values, dp_values, q_values)
