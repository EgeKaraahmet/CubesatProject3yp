...
%% At h= 10 km
target_h = 10 * 10^3; 
target_X = find_corr_value(h_reference_signal, x_reference_signal, target_h);
target_V = find_corr_value(h_reference_signal, V_reference_signal, target_h);
target_theta = find_corr_value(h_reference_signal, theta_reference_signal, target_h);
target_gamma = find_corr_value(h_reference_signal, gamma_reference_signal, target_h);


% xf = [target_h; target_X; target_V; target_theta; target_gamma];
% Combine stage parameters into a column array.
pvcost = [xf; Sf; Q; R; Q_heat; p];
...
% Xopt = x_plot_out;
% [p_adapted,~] = size(Xopt);
% Xref = [Xopt(1:p_adapted,:);repmat(Xopt(end,:),Tsteps-p_adapted,1)];
% 
% 
% 
% %%
...
%     [uk,options] = nlmpcmove(msobj_tracking,xk,lastMV,Xref(k:min(k+9,Tsteps),:),[],options);
...
% V_plot_kmf = V_plot(positive_indices_kmf);
% theta_plot_kmf = theta_plot(positive_indices_kmf);
% gamma_plot_kmf = gamma_plot(positive_indices_kmf);
% u_plot_kmf = u_plot(positive_indices_kmf);
% 
% x_plot_out_kmf = [h_plot_kmf X_plot_kmf V_plot_kmf theta_plot_kmf gamma_plot_kmf];
% 
% 
% 
% %%
...
% %CubeSatPlotTracking(Xopt, Ts, p, Tsteps, xHistory_kmf, uHistory_kmf);
% 
