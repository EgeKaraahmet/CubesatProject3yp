...
% Input: 
% u: fin areas
x0 = [100;(200+6378)*10^3;7788;7788];
u0 = 0.5;
...
planner = nlmpcMultistage(pPlanner,4,1);
planner.Ts = Ts;

%%
% Specify prediction model and its analytical Jacobian.
planner.Model.StateFcn = 'CubeSatStateFcn_23122023';
planner.Model.StateJacFcn = 'CubeSatStateJacobianFcn_23122023';
...
% final stage, |p+1|.
for ct=1:pPlanner
    planner.Stages(ct).CostFcn = 'CubeSatStateFcn_23122023';
    planner.Stages(ct).CostJacFcn = 'CubeSatStateJacobianFcn_23122023';
end

%% 
% To ensure a successful landing at the target, specify terminal state for
% the final stage. 
planner.Model.TerminalState = [0;0;0;0];
...
validateFcns(planner,rand(4,1),rand(1,1));
