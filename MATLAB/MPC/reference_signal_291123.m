...
kepOut = sim('Copy_of_reference_signal_generator_MPC','Solver',solv_kep,'FixedStep',step_kep);
...
atmOut = sim('Copy_of_reference_signal_generator_MPC','Solver',solv_atm,'FixedStep',step_atm,'StartTime','kepOut.time(end)');
...
h_end_125=atmOut.h(end);         %m        %altitude
x_end_125=atmOut.x(end);         %m        %travelled distance
V_end_125=atmOut.V(end);            %m/s      %orbital speed
theta_end_125=atmOut.theta(end);    %rad      %true anomaly
gamma_end_125=atmOut.gamma(end);    %rad      %flight path angle


h_atm_125 = [atmOut.h]; 
X_atm_125 = [atmOut.x];
plot(X_atm_125,h_atm_125)
title('A = 125cm^2')
%   data filtering
