...
kepOut = sim('controller_dynamics','Solver',solv_kep,'FixedStep',step_kep);
...
atmOut = sim('controller_dynamics','Solver',solv_atm,'FixedStep',step_atm,'StartTime','kepOut.time(end)');
...
fprintf('Area: %3.2f cm^2\n',A*10^4)
fprintf('Total distance travelled: %3.2f km\n',x_reference_signal(end)/1000)

no_of_orbiting = floor(abs(x_reference_signal(end)/(2*pi*Re)));
fprintf('Total no. of completed orbiting around the Earth: %3.2f\n', no_of_orbiting)

dist_to_travel = (abs(x_reference_signal(end)) - no_of_orbiting * 2*pi*Re)/1000;
fprintf('To be landed distance: %3.2f km\n',dist_to_travel)
...
%aerothermal load
q_dot=1.83e-4*V_reference_signal.^3.*sqrt(rho_reference_signal./rcurv);
q_dot_max=max(q_dot);
h_q_dot_max=h_reference_signal(q_dot==q_dot_max)/1000;
fprintf('Max heat flux of %3.2e W/m^2 experienced at an altitude of %.1fkm\n',q_dot_max,h_q_dot_max)
...
figure
plot(x_reference_signal,h_reference_signal)
...
figure
plot(time_reference_signal.*60./3.154e+7*365,q_dot)
xlabel('time [days]')
ylabel('heat flux [W/m^2]')
grid on
grid minor
