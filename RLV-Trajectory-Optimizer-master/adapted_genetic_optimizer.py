...
from adapted_empirical_data import database
...
class optimizer:
    ...
    prob_mt = 0.2  # probability of mutation
    ...
    def eval(self, x):
        ...
        weights = [0.3, 0.7]    ## [0.8, 0.5]
        functions = [Qp_kpa, q]
        # weighted sum average
        fit = sum(x * y for x, y in zip(weights, functions)) / sum(functions) # why sum(functions)
        #fit = sum(weights[i]*f for i, f in enumerate(functions) )
        return fit,

    # constraints
    # (1) Qp < 40 kPa         # Dynamic pressure
    # (2) q < 200 W/cm^2     # Heat flux

    def feasible(self, x):
        alpha = x[0]
        alpha, L, D, Qp_kpa, q = self.calculate(alpha)
        # enforce constraints
        if q <= 200 and Qp_kpa <= 40:
            return True  # Constraints are satisfied
        return False  # Constraints are violated
    ...
    def get_values(self, y, ctrl_data, orb_data, tem_data, aero_data):
        ...
        [alpha, L, D] = ctrl_data[-1]  ## [alpha, L, D]
        [Rn, m, rho0] = orb_data
        [R, K, T0]    = tem_data
        [S, k]        = aero_data
        self.y = y[-1]
